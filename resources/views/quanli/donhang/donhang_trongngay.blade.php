				...
				<table id="table-donhang-admin" class="table table-bordered table-hover">
				    ...
				    <tbody>
				    	<?php
				    		$t = 0;
				    		foreach ($dh as $valdh) {
				    			if(strtotime(date('Y-m-d',strtotime($ngayht))) == strtotime($valdh->ngaydat)){
				    				$t +=1;
				    			}
				    		}
				    		if($t == 0){ ?>
				    			<tr>
			                        <td align="center" colspan="9" style="color: red"><h4>Không có đơn hàng trong ngày !</h4></td>
			                    </tr>
				    		<?php }else{ 
				    			foreach ($dh as $val) {
				    ...
				    <tfoot>
				    	<tr>
				    		<td align="center" colspan="8">{!! $dh->render() !!}</td>
				    	</tr>
				    </tfoot>
				</table>
